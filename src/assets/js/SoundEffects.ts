...
export default class SoundEffects {
  ...
  public play(durationInSecond: number): Promise<boolean> {
    if (this.isMuted) {
      return Promise.resolve(false);
    }
    ...
    this.playSound(
      Array.from(Array(durationInSecond * 600 ), (_, index) => musicNotes[index % 3]),
      { type: 'triangle', easeOut: false, volume: 1 }
    );

    return new Promise<boolean>((resolve) => {
      setTimeout(() => {
        resolve(true);
      }, durationInSecond * 600);
    });
  }
